...
GRAPH.calc.interpreter.expression.CalcExpressionFactory = (function() {
	...
	class CalcExpressionFactory {
		...
		static createExpression(calcToken) {
			...
			let VariableExpression = GRAPH.calc.interpreter.expression.VariableExpression;
			...
			if (calcToken !== null) {
				...
				else if (calcToken.getType() === CalcToken.VARIABLE) {
					expression = new VariableExpression(calcToken.getToken());
				}
			}
