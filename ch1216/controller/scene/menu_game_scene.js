...
GAME.controller.scene.MenuGameScene = (function() {
	...
	class MenuGameScene extends ImplGameScene {
		...
		press(context, mouseX, mouseY) {	
			if(this.startSprite.contains(mouseX, mouseY)) {
				context.changeScene(GAME.controller.scene.PlayGameScene.getInstance());
			}
			
			if(this.configSprite.contains(mouseX, mouseY)) {
				context.changeScene(GAME.controller.scene.ConfigGameScene.getInstance());
			}
		}
