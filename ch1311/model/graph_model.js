...
GRAPH.model.GraphModel = (function() {
	...
	let GraphPoint = GRAPH.struct.GraphPoint;
			...
			try {
				...
				if (expression !== null) {
					...
					for (let i = this.minXValue; i <= this.maxXValue; i++) {
						...
						if (answer !== GraphConstants.INVALID_NUMBER) {
							if (answer === GraphConstants.INFINITE_NUMBER) {
								...
								this.points.push(new GraphPoint(x, y, false));
								...
								this.points.push(new GraphPoint(x, y, true));
							}
							else {
								y = parseInt(answer);
